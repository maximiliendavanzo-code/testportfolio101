...
def inject_glass_theme(accent_key: str) -> None:
    ...
    .glass-icon {{
        background:
            radial-gradient(circle at 30% 25%, rgba(255,255,255,0.75) 0%, rgba(255,255,255,0) 42%),
            linear-gradient(160deg, rgba({accent_rgb},0.38), rgba({accent_rgb},0.16));
        backdrop-filter: blur(16px) saturate(180%);
        -webkit-backdrop-filter: blur(16px) saturate(180%);
        border: 1px solid rgba(255,255,255,0.55);
        box-shadow: 0 8px 20px rgba({accent_rgb},0.25), inset 0 1px 1px rgba(255,255,255,0.7);
        border-radius: 50%;
    }}

    input[type="range"] {{ accent-color: {accent} !important; }}

    div[class*="st-key-lang_selector"] {{ margin-top: -60px !important; }}
...
def render_tool_card(name: str, suffix: str, accent_key: str, description: str,
    ...
    if icon:
        icon_html = (
            f'<div class="glass-icon" style="width:48px; height:48px; '
            f'display:flex; align-items:center; justify-content:center;">{render_icon(icon, '#ffffff', 22)}</div>'
        )
...
def render_stats_bar(items: list) -> None:
    ...
    blocks = "".join(f"""
        <div style="display:flex; align-items:center; gap:12px;">
          <div class="glass-icon" style="width:48px; height:48px; display:flex; align-items:center;
                      justify-content:center; flex-shrink:0;">
            {render_icon(icon_name, '#ffffff', 20)}
          </div>
          <div>
...
def render_how_it_works(title: str, steps: list, explication_label: str = "Explication", ex_label: str = "Ex") -> None:
    """steps : liste de tuples (titre, explication, exemple), affichés en 3 étapes numérotées reliées par un trait."""
    accent_rgb = _hex_to_rgb(PURPLE)
    n = len(steps)
    items_html = "".join(f"""
        <div style="flex:1; position:relative; z-index:1; padding:0 18px; min-width:180px; text-align:center;">
          <div style="width:40px; height:40px; border-radius:50%; background:#fbfaff;
                      border:2px solid {PURPLE}; color:{PURPLE}; font-weight:800; font-size:15px;
                      display:flex; align-items:center; justify-content:center; margin:0 auto 12px;">{i+1}</div>
          <div style="font-weight:700; font-size:15px; color:{INK}; margin-bottom:6px;">{step_title}</div>
          <div style="font-size:13px; color:{TEXT_SECONDARY}; line-height:1.5; margin-bottom:4px;">
            <b style="color:{INK};">{explication_label} :</b> {explication}
          </div>
          <div style="font-size:13px; color:{TEXT_SECONDARY}; line-height:1.5;">
            <b style="color:{INK};">{ex_label} :</b> {exemple}
          </div>
        </div>
        """ for i, (step_title, explication, exemple) in enumerate(steps))
    _html(f"""
    <div style="text-align:center; font-weight:700; font-size:20px; color:{INK}; margin:80px 0 26px !important;">{title}</div>
    <div style="position:relative; display:flex; flex-wrap:wrap; justify-content:space-between; max-width:1000px; margin:0 auto 30px;">
      <div style="position:absolute; top:20px; left:16.6%; right:16.6%; height:0;
                  border-top:2px dashed rgba({accent_rgb},0.35); z-index:0;"></div>
      {items_html}
    </div>
    """)
