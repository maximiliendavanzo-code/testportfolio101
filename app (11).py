...
from theme import inject_glass_theme, render_tool_card, render_ring_metric, render_clickable_card, render_clickable_logo
...
def render_finance_home():
    ...
    tools = [
        ("Portfolio", "", "portfolio", "portfolio", t("tool_portfolio_desc"), True),
        ("CIB", "", "cib", "cib", t("tool_cib_desc"), False),
        ("M&A", "", "ma", "ma", t("tool_ma_desc"), False),
    ]
    cols = st.columns(3)
    for col, (name, suffix, accent_key, page_key, desc, available) in zip(cols, tools):
        with col:
            clicked = render_clickable_card(
                name, suffix, accent_key, desc, key=page_key,
                on_click_page=page_key if available else None,
                badge=None if available else t("coming_soon_badge"),
            )
            if clicked:
...
def render_coming_soon(logo_file: str):
    if render_clickable_logo(logo_file, 280, key="comingsoon"):
...
def render_ma_page():
    ...
    if render_clickable_logo("logo_ma101.svg", 220, key="ma"):
        st.session_state["page"] = "finance_home"
        st.rerun()
...
def render_portfolio_page():
    if render_clickable_logo("logo_wordmark.svg", 260, key="portfolio"):
        st.session_state["page"] = "finance_home"
        st.rerun()
