...
from theme import (
    inject_glass_theme, render_tool_card, render_ring_metric,
    render_bg_shapes, render_stats_bar, render_hero_illustration, render_how_it_works, render_footer,
)
...
def render_finance_home():
    col_text, col_illu = st.columns([5, 4], gap="large")
    with col_text:
    ...
    render_stats_bar([
        ("chart", t("stat_1_label"), t("stat_1_caption")),
        ("database", t("stat_2_label"), t("stat_2_caption")),
        ("shield", t("stat_3_label"), t("stat_3_caption")),
    ])
    ...
    for col, (name, suffix, accent_key, page_key, desc, icon, tags, cta, available) in zip(cols, tools):
        with col:
            render_tool_card(name, suffix, accent_key, desc, icon=icon, tags=tags,
                              badge=t("coming_soon_badge") if not available else None)
            st.write("")
...
def render_portfolio_page():
    ...
    with ring_cols[0]:
        render_ring_metric(f"{result['cagr']*100:+.1f}%", result["cagr"], -0.30, 0.30,
                            t("metric_cagr").upper(), "portfolio", help_text=t("metric_cagr_help"))
    with ring_cols[1]:
        render_ring_metric(f"{result['volatility_annualized']*100:.1f}%", result["volatility_annualized"], 0.0, 0.40,
                            t("metric_vol").upper(), "portfolio", help_text=t("metric_vol_help"))
    with ring_cols[2]:
        render_ring_metric(f"{result['sharpe']:.2f}", result["sharpe"], -1.0, 3.0,
                            t("metric_sharpe").upper(), "portfolio", help_text=t("metric_sharpe_help"))
    st.write("")
