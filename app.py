...
st.set_page_config(page_title="Analyseur de portefeuille", page_icon="📊", layout="wide")

st.markdown("""
<style>
    .stMetric { background: white; border: 1px solid rgba(0,0,0,0.08); border-radius: 12px; padding: 10px; }
    div[data-testid="stMetricValue"] { font-family: 'IBM Plex Mono', monospace; }
</style>
""", unsafe_allow_html=True)

st.title("📊 Analyseur de portefeuille")
...
if st.session_state.get("analyzed"):
    ...
    fig.add_trace(go.Scatter(x=result["portfolio_value"].index, y=result["portfolio_value"],
                              name="Portefeuille", line=dict(width=3, color="#1D4E89")))
    ...
    with col_right:
        ...
        fig_hist.add_vline(x=-result["var_historical"] * 100, line_color="#B23B3B", line_dash="dash",
                            annotation_text="VaR")
        fig_hist.update_layout(height=380, showlegend=False, margin=dict(l=10, r=10, t=10, b=10))
