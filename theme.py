...
clés. Basé sur la direction validée dans Claude Design (Source Serif 4 + Manrope).
"""
...
def inject_glass_theme(accent_key: str) -> None:
    ...
    st.markdown(f"""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Source+Serif+4:opsz,wght@8..60,500;8..60,600;8..60,700&family=Manrope:wght@400;500;600;700;800&display=swap');
...
def render_tool_card(name: str, suffix: str, accent_key: str, description: str, badge: str = None) -> None:
    ...
    <div class="glass-card" style="box-shadow:0 8px 32px rgba({accent_rgb},0.15);">
      <div>
        <div style="font-family:'Source Serif 4',serif; font-weight:700; font-size:20px; color:{INK};">
          {name}<span style="color:{accent};">{suffix}</span>
        </div>
        <div style="width:32px; height:3px; background:{accent}; border-radius:2px; margin-top:4px;
                    box-shadow:0 0 10px rgba({accent_rgb},0.5);"></div>
      </div>
    ...
    <div title="{title_attr}" style="display:flex; align-items:center; gap:14px; background:rgba(255,255,255,0.5);
      ...
      <div style="width:80px; height:80px; border-radius:50%; flex-shrink:0;
        ...
        <div style="width:58px; height:58px; border-radius:50%; background:#fbfaff;
                    display:flex; align-items:center; justify-content:center;
                    font-family:'Manrope',sans-serif; font-weight:800; font-size:14px; color:{INK};">
          {value_str}
        </div>
      </div>
