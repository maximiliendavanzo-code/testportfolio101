...
def inject_glass_theme(accent_key: str) -> None:
    ...
    /* cartes/logos entièrement cliquables : bouton invisible superposé sur un conteneur nommé "card_..." ou "logoback_..." */
    div[class*="st-key-card_"], div[class*="st-key-logoback_"] {{ position: relative; }}
    div[class*="st-key-card_"] div[data-testid="stButton"],
    div[class*="st-key-logoback_"] div[data-testid="stButton"] {{
        position: absolute; inset: 0; z-index: 5; margin: 0 !important;
    }}
    div[class*="st-key-card_"] div[data-testid="stButton"] button,
    div[class*="st-key-logoback_"] div[data-testid="stButton"] button {{
        width: 100%; height: 100%; opacity: 0; cursor: pointer;
        border: none !important; background: transparent !important; box-shadow: none !important;
    }}
    div[class*="st-key-card_"]:hover .glass-card {{
    ...
    div[class*="st-key-logoback_"] {{ display: inline-block; border-radius: 10px; transition: background 0.2s; }}
    div[class*="st-key-logoback_"]:hover {{ background: rgba({accent_rgb},0.08); }}
...
def render_clickable_card(name: str, suffix: str, accent_key: str, description: str,
                           key: str, on_click_page: str = None, badge: str = None) -> str:
    """
    Carte en verre entièrement cliquable (bouton invisible superposé). Retourne la nouvelle
    valeur de page si la carte a été cliquée pendant ce run, sinon None.
    Si badge est fourni (ex: "Bientôt disponible"), la carte n'est pas cliquable.
    """
    clicked_page = None
    if badge:
        render_tool_card(name, suffix, accent_key, description, badge=badge)
    else:
        with st.container(key=f"card_{key}"):
            render_tool_card(name, suffix, accent_key, description)
            if st.button("", key=f"open_{key}"):
                clicked_page = on_click_page
    return clicked_page


def render_clickable_logo(svg_path: str, width: int, key: str) -> bool:
    """Affiche le logo et le rend cliquable (retour à l'accueil). Retourne True si cliqué."""
    clicked = False
    with st.container(key=f"logoback_{key}"):
        st.image(svg_path, width=width)
        if st.button("", key=f"logobtn_{key}"):
            clicked = True
    return clicked
