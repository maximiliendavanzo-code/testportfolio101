...
def _html(body: str) -> None:
    """
    st.markdown(unsafe_allow_html=True) mais en retirant d'abord l'indentation de chaque
    ligne : sinon Streamlit (Markdown GitHub-flavored) peut interpréter les lignes indentées
    de 4 espaces ou plus comme un bloc de code brut, et afficher le HTML tel quel au lieu
    de le rendre.
    """
    compact = re.sub(r"\n[ \t]+", "", body).strip()
...
def render_hero_illustration(title: str, label_return: str, value_return: str,
                              label_vol: str, value_vol: str, label_alloc: str, label_risk: str) -> None:
    """Illustration stylisée façon aperçu produit (panneau principal + 2 cartes flottantes), en verre."""
    accent_rgb = _hex_to_rgb(PURPLE)
    line_points = "0,55 20,45 40,50 60,25 80,32 100,10 120,20 140,5"
    bars = "".join(f'<div style="width:10px; height:{h}px; background:{PURPLE}; border-radius:3px;"></div>'
                    for h in [14, 22, 17, 28])
    _html(f"""
    <div style="position:relative; height:320px; margin-top:10px;">
      <div style="position:absolute; top:0; left:0; width:150px; background:rgba(255,255,255,0.55);
                  backdrop-filter:blur(14px); border:1px solid rgba(255,255,255,0.7); border-radius:16px;
                  padding:14px; box-shadow:0 10px 30px rgba({accent_rgb},0.12);">
        <div style="font-size:11px; color:{TEXT_SECONDARY}; margin-bottom:10px;">{label_alloc}</div>
        <div style="width:56px; height:56px; border-radius:50%; margin:0 auto;
                    background:conic-gradient({PURPLE} 0deg 216deg, rgba({accent_rgb},0.55) 216deg 288deg, rgba({accent_rgb},0.25) 288deg 360deg);"></div>
      </div>

      <div style="position:absolute; top:34px; left:110px; right:20px; background:rgba(255,255,255,0.55);
                  backdrop-filter:blur(14px); border:1px solid rgba(255,255,255,0.7); border-radius:18px;
                  padding:18px; box-shadow:0 14px 40px rgba({accent_rgb},0.15);">
        <div style="display:flex; gap:6px; margin-bottom:12px;">
          <span style="width:8px;height:8px;border-radius:50%;background:#e5867a;display:inline-block;"></span>
          <span style="width:8px;height:8px;border-radius:50%;background:#e8c67a;display:inline-block;"></span>
          <span style="width:8px;height:8px;border-radius:50%;background:#8fcf8a;display:inline-block;"></span>
          <span style="font-size:12px; color:{TEXT_SECONDARY}; margin-left:8px;">{title}</span>
        </div>
        <svg width="100%" height="70" viewBox="0 0 140 60" preserveAspectRatio="none">
          <polyline points="{line_points}" fill="none" stroke="{PURPLE}" stroke-width="3"
                    stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        <div style="display:flex; gap:10px; margin-top:14px;">
          <div style="flex:1; background:rgba({accent_rgb},0.08); border-radius:10px; padding:8px 10px;">
            <div style="font-size:10px; color:{TEXT_SECONDARY};">{label_return}</div>
            <div style="font-size:15px; font-weight:800; color:#2f8f5b;">{value_return}</div>
          </div>
          <div style="flex:1; background:rgba({accent_rgb},0.08); border-radius:10px; padding:8px 10px;">
            <div style="font-size:10px; color:{TEXT_SECONDARY};">{label_vol}</div>
            <div style="font-size:15px; font-weight:800; color:{INK};">{value_vol}</div>
          </div>
        </div>
      </div>

      <div style="position:absolute; bottom:0; right:10px; width:130px; background:rgba(255,255,255,0.55);
                  backdrop-filter:blur(14px); border:1px solid rgba(255,255,255,0.7); border-radius:16px;
                  padding:14px; box-shadow:0 10px 30px rgba({accent_rgb},0.12);">
        <div style="font-size:11px; color:{TEXT_SECONDARY}; margin-bottom:10px;">{label_risk}</div>
        <div style="display:flex; align-items:flex-end; gap:6px; height:32px;">{bars}</div>
      </div>
    </div>
    """)
