...
from ma_export import build_ma_excel_report, build_ma_pdf_report
...
from theme import (
    inject_glass_theme, render_tool_card,
    render_bg_shapes, render_hero_illustration, render_how_it_works, render_footer,
)
...
_PAGE_ACCENT = {"finance_home": "finance", "portfolio": "portfolio", "ma": "ma", "cib": "cib"}
inject_glass_theme(_PAGE_ACCENT.get(st.session_state["page"], "finance"))
render_bg_shapes()
...
def render_finance_home():
    col_text, col_illu = st.columns([5, 4], gap="small")
    with col_text:
    ...
    with col_illu:
        render_hero_illustration(
            t("hero_panel_title"), t("hero_return_label"), t("hero_return_value"),
            t("hero_vol_label"), t("hero_vol_value"), t("hero_alloc_label"), t("hero_risk_label"),
        )

    tools = [
        ("Portfolio", "", "portfolio", "portfolio", t("tool_portfolio_desc"), "portfolio",
         [t("tag_correlation"), t("tag_volatility"), t("tag_var"), t("tag_es")], t("cta_portfolio"), True),
        ("CIB", "", "cib", "cib", t("tool_cib_desc"), "cib",
         [t("tag_valuation"), t("tag_debt"), t("tag_financing")], t("cta_cib"), False),
        ("M&A", "", "ma", "ma", t("tool_ma_desc"), "ma",
         [t("tag_multiples"), t("tag_synergies"), t("tag_dealanalysis")], t("cta_ma"), False),
    ]
    cols = st.columns(3)
    for col, (name, suffix, accent_key, page_key, desc, icon, tags, cta, available) in zip(cols, tools):
        with col:
            render_tool_card(name, suffix, accent_key, desc, icon=icon, tags=tags)
            st.write("")
            if st.button(cta, key=f"open_{page_key}", use_container_width=True):
                st.session_state["page"] = page_key
                st.rerun()

    st.write("")
    render_how_it_works(t("howitworks_title"), [
        (t("howitworks_1_title"), t("howitworks_1_explication"), t("howitworks_1_ex")),
        (t("howitworks_2_title"), t("howitworks_2_explication"), t("howitworks_2_ex")),
        (t("howitworks_3_title"), t("howitworks_3_explication"), t("howitworks_3_ex")),
    ], explication_label=t("label_explication"), ex_label=t("label_ex"))
    render_footer(t("footer_text"))


def render_coming_soon(logo_file: str):
...
def render_ma_page():
    ...
    with tab_comps:
        ...
        if peer_tickers:
            ...
            st.dataframe(df_peers.style.format({"ev_ebitda": "{:.1f}x", "ev_revenue": "{:.1f}x", "pe": "{:.1f}x"}, na_rep="n.a."),
        ...
        if not valuation_ranges:
            ...
            for label in labels:
                ...
                fig.add_trace(go.Scatter(x=[low, high], y=[label, label], mode="lines+markers",
                                          line=dict(color="#6B2FBF", width=8), marker=dict(size=10, color="#6B2FBF"), showlegend=False))
                fig.add_trace(go.Scatter(x=[mid], y=[label], mode="markers",
                                          marker=dict(size=12, color="#111111", symbol="line-ns-open"), showlegend=False))
            if target["price"]:
                fig.add_vline(x=target["price"], line_dash="dash", line_color="#4b4f5e", annotation_text=t("ma_ff_current_price"))
    ...
    if dcf_result is None:
        st.info(t("ma_export_needs_dcf"))
    else:
        syn_mode_label = "cost" if syn_mode in (None, t("ma_syn_mode_cost")) else "revenue"
        ff_png = None
        if valuation_ranges:
            try:
                ff_png = fig.to_image(format="png", width=900, height=400, scale=2)
            except Exception:
                ff_png = None

        exp_col1, exp_col2, exp_col3 = st.columns(3)
        with exp_col1:
            excel_bytes = build_ma_excel_report(
                target, acquirer, dcf_result, sens,
                comps_result["valuation"] if comps_result else None,
                trans_result["valuation"] if trans_result else None,
                syn_result, syn_mode_label,
                valuation_ranges, offer_result, deal, tranches, blended_rate, eps_result, leverage,
            )
            st.download_button(t("export_excel"), data=excel_bytes, file_name="valorisation_ma101.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True)

        with exp_col2:
            if ff_png:
                st.download_button(t("export_png"), data=ff_png, file_name="football_field.png",
                                    mime="image/png", use_container_width=True)
            else:
                st.button(t("export_png"), disabled=True, use_container_width=True, key="ma_png_disabled")

        with exp_col3:
            pdf_bytes = build_ma_pdf_report(target, offer_result, valuation_ranges, dcf_result, eps_result, ff_png)
            st.download_button(t("export_pdf"), data=pdf_bytes, file_name="rapport_ma101.pdf",
